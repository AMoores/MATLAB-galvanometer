function [OffsetMagnitude,TotalUnc,Offsetx,Offsety,UncX,UncY, AllCounts, AllChosen, Unit, offsetxpix, offsetypix] = OffsetMain()
...
%the count image. Offset calculation is performed on the newest capture file,
...
%LabVIEW alongside the MATLAB program, always sending the most up-to-date 
%value to the Galvo via a USB-DAQ.
...
while ishandle(hmsg)
    ...
    [Counts,Alt] = ImageFilters(Counts,Alt);   %filters the images to remove banding and gradients
    
    
    %----------------------------------------------------
    if loops == 1
        [Bin_function] = BinarizeImages(Counts, Alt);   
    end
    %On the first iteration, the images are passed through an image
    %processing selector, where both images are binarised according to the
    %Otsu and Edge Detection algorithms - user is asked which process has
    %given the most similar result across the two different images - this
    %process will be used automatically for the remaining iterations.
    
    if Bin_function == 1
         [Bin_Counts,thresh_pos1] = OtsuThreshold(Counts,50);
         [Bin_Alt,thresh_pos2] = OtsuThreshold(Alt,50);
    else
        Bin_Counts = EdgeDetect(Counts);
        Bin_Alt = EdgeDetect(Alt);
    end

    %-----------------------------------
    
    [Realoffsetx, Realoffsety, A, cor, calcUncX, calcUncY,offsetxpix,offsetypix] = CrossCorrelateTest(Counts,Alt,Bin_Counts, Bin_Alt, Image2name, RLlength);   %performs the cross-correlation measurement
